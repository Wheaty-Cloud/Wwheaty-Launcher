![Wwheatylauncher Logo](icone.png)

Wwheatylauncher
=============
Wwheaty launcher is a 3rd party minecraft launcher in python !

# Installation

Go to [the releases][Releases]
 and select the version you want (we recommend you use the latest or recommended release).

# Microsoft login

Upon launching the exe, you can click the button to register your Microsoft account to verify if you own the game in order to be able to play it.

# Installing a version

## Vanilla

In the main window, click the "Install version" button at the top. In the new window, click the vanilla tabs. You can then choose releases, snapshots, betas or alphas, then the version  in the drop down menus.

## Modded versions

In the installation window, you can choose your mod loader between NeoForge, Forge and Fabric. Then, type the minecraft version in the text field and choose the loader version in the dropdown menu.

# Using Mods

Once you have set up your modded installation, you can add mods very simply by using the mod manager. To do so, click the "Mod manager" button, then add the mods you want in the new window, and they will automatically be used when using a mod installation. You can also disable or delete your mods from there or installing mods in the integrate modrinth window
.

[Releases]: https://github.com/Wheaty-Cloud/Wwheaty-Launcher/releases/
